...
if (type is UserDefinedType) {
        ...
        foreach (var decl in ModuleDefinition.AllIteratorDecls(moduleDef.TopLevelDecls)) {
         if (decl.ToString() == typeStr) {
            var moduleName = GetFullModuleName(moduleDef);
            var result = (moduleName == "") ? typeStr : (moduleName + "." + typeStr);
            if (type.TypeArgs.Count != 0) {
...
//     }
      ...
      if (expr is StaticReceiverExpr) {
        ...
        */
        ...
        if (!skipSignatureParens) { wr.Write("("); }
         if(Prefix != "")
        {
          wr.Write(Util.Comma(e.BoundVars, bv => Prefix+bv.DisplayName + (ShowType(bv.Type) ? ": " + bv.Type : "")));
        }else{
          wr.Write(Util.Comma(e.BoundVars, bv => bv.DisplayName + (ShowType(bv.Type) ? ": " + bv.Type : "")));
    ...
    private void PrintQuantifierDomain(List<BoundVar> boundVars, Attributes attrs, Expression range) {
      ...
      foreach (BoundVar bv in boundVars) {
        if(Prefix != "")
        {
          wr.Write("{0}{1}", sep, Prefix+bv.DisplayName);
        }else{
