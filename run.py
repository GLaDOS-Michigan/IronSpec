...
nodes_list=['003', '004', '007', '008', '009', '013', '017', '018', '023', '024', '031', '032', '035', '036', '038', '039', '053', '056', '064', '070', '087', '093', '094', '095']
...
for i in np.arange(len(hole_list)):
    node_id=i % len(nodes_list)
    # print(i, node_id, nodes_list[node_id], hole_list[i])
    tasks[node_id].append(hole_list[i])
for i in np.arange(len(nodes_list)):
    # print(i, tasks[i])
    cmd = '/proj/vm-vm-PG0/BASE-DIRECTORY/dafny-holeEval/run_hole_eval.sh '
    for t in tasks[i]:
        cmd += t + ' '
    cmd += '& disown -a'
